export const canvas = document.getElementById('canvas');
export const svg = document.getElementById('links');
export const tabBar = document.getElementById('tabs');
export const addTabBtn = document.getElementById('addTabBtn');
export const addAgentBtn = document.getElementById('addAgentBtn');
export const addGoalBtn = document.getElementById('addGoalBtn');
export const addBlockerBtn = document.getElementById('addBlockerBtn');
export const addFollowupBtn = document.getElementById('addFollowupBtn');
export const playBtn = document.getElementById('playBtn');

export const nodes = new Map();
export const edges = [];